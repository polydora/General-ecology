...
class <- read_excel(path = "data/Classess_2024_biology.xls", col_names = T)
...
# Тест по Лекции 1
test_lecture_1 <- read_excel("data/Общая экология 2024-2025 grades - 09_16_2024, 08_12 PM.xlsx", col_names = TRUE)

test_lecture_1 <- test_lecture_1 %>% select(c(4,10))
...
#
# test_lecture_4 <- read_xlsx("data/Конкуренция (Экология для биоинженеров 2024)(1-13).xlsx")
...
<!-- hw2 <- read_xlsx("data/Контрольная работа № 2 (Экология для биоинженеров 2024)(1-13).xlsx") -->
<!-- hw2 <- hw2 %>% select(4, 6) -->
...
<!-- hw2_score$Exam <- round(hw2_score$Score / hw2_score$Total * 3, 1) -->
...
<!-- cont2 <- read_excel("data/Контрольная работа № 2 (Экология для биоинженеров 2024)(1-13).xlsx") -->

<!-- cont2 <- cont2 %>% select(4:6) -->

<!-- names(cont2) <- c("St_Mail",  "Name", "Score") -->

<!-- cont2_print <- merge(class, cont2 %>% select(-Name ), all.x = TRUE ) %>% select(-St_Mail) %>% mutate(Exam = round(Score / 13 * 3, 1)) -->
...
<!-- seminars <- read_excel("data/Seminars_biotech.xlsx") -->
<!-- #  -->
...
<!-- res <- merge(class, final) -->

<!-- write.table(res, "data/results_for_examen_2024_biotech.csv", sep = ";", row.names = F, fileEncoding = "UTF-8", dec = "," ) -->
