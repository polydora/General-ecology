...
cont_reg <- read_excel("data/Регистрационная форма для выполнения контрольной работы № 1 (Responses).xlsx")

cont <- read_excel("data/Контрольная работа №1 (Responses).xlsx")
...
Pl_lect <- ggplot(credits_print, aes(x = Exam)) + stat_count(geom = "col") + labs(x = "% от экзаменационной оценки") + ggtitle("Тесты по материалам лекций")

Pl_hw1 <- ggplot(hw1_score, aes(x = Exam)) + stat_count(geom = "col") + labs(x = "% от экзаменационной оценки") + ggtitle("Самостоятельная работа №1")


Pl_cw1 <- ggplot(cont1, aes(x = Exam)) + stat_count(geom = "col") + labs(x = "% от экзаменационной оценки") + ggtitle("Контрольная работа №1")

Pl_hw2 <- ggplot(hw2_score, aes(x = Exam)) + stat_count(geom = "col") + labs(x = "% от экзаменационной оценки") + ggtitle("Самостоятельная работа №2")


(Pl_lect + Pl_hw1)/(Pl_cw1 + Pl_hw2)
...
```{r}
seminars <- read_excel("data/Семинары по общей экологии.xlsx")
Groups <- seminars %>% select(c(Group))

seminars <- seminars %>% select(-c(Family, Name, Group))

seminars$Sum <- rowSums(seminars[, 2:8], na.rm = T)
