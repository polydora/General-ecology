...
Коллеги! В настоящий момент большая часть курса зарегистрировалась в системе Ms Teams, которая будет использоваться для администрирования и проведения тестов. Однако есть несколько человек, которые значатся на курсе, но пока не зарегистрировались. Если их ID не станет известен в ближайшее время, то за первый тест у них не будут начислены баллы.

Вот эти люди.   
Если Вы есть среди них, то обязательно свяжитесь с В. М. Хайтовым в ближайшее время.

```{r,  echo=FALSE, message=FALSE, warning=FALSE}
library(knitr)
library(dplyr)

class <- read.csv("data/Classess_2020.csv")

class <- class[, 1:7]

teams <- read.csv("data/teams.csv")

dolg <- class %>% filter(!(St_Mail %in% teams$Email)) %>% select(Family, Name, Otch ) 

options(knitr.kable.NA = '')
kable(dolg, col.names = c("Фамилия", "Имя", "Отчество"))
```
