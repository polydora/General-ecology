...
Pl_hw1 <- ggplot(hw1_score, aes(x = Exam)) + stat_count(geom = "col") + labs(x = "% от экзаменационной оценки") + ggtitle("Тесты по самостоятельной работt №1")


Pl_cw1 <- ggplot(cont1, aes(x = Exam)) + stat_count(geom = "col") + labs(x = "% от экзаменационной оценки") + ggtitle("Контрольная работа №1")

Pl_foo <- ggplot() + theme(panel.background = element_blank())
(Pl_lect + Pl_hw1)/(Pl_cw1 + Pl_foo)
