...
Внимание! В одном из вопросов (про эффект Олли) был технический сбой. Поэтому этот вопрос не будет учитываться, общее количество вопросов будет 14.
...
cont2_print <- merge(class, cont2 %>% select(-Name ) ) %>% select(-St_Mail) %>% mutate(Exam = round(Score / 14*1.5, 1))
