...
names(test_lecture_4) <- c("St_Mail", "Score_lecture_5")
...
<!-- test_lecture_6 <- read_xlsx("data/Трофические связи (Общая экология для биоинженеров 2022) (1-27).xlsx") -->
...
<!-- test_lecture_7 <- read_xlsx("data/Конкуренция и трофические связи (Общая экология для биоинженеров 2022)(1-26).xlsx") -->
...
Коллеги! Я приношу свои извинения за ошибку в вопросе про накопленную сумму температур. Те, кто не ответил на этот вопрос, поступили единственно правильным способом. Поэтому им я добавил один балл. Всем остальным я ответы на этот вопрос не зачел, но (!) общее количество вопросов сократил на один. Так что никто ничего не потерял, а те, кто не ответил на "кривой" вопрос, смогли получить даже больше 1.5% от финальной оценки.
...
hw1 <- read_xlsx("data/Тест по самостоятельной работе №1 (Общая экология 2022)(1-98).xlsx")
hw1 <- hw1 %>% select(1, 3)
...
kable(hw1_score_print, col.names = c("Login", "Сумма",  "% от экзаменационной оценки"))
