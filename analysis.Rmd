...
test_lecture_2$Score <- test_lecture_2$Score + 1 
...
#Тест по Лекции 5
test_lecture_5 <- read_xlsx("data/Взаимоотношения. Лекция 5. (2)(1-26).xlsx")
...
# 
# #Тест по Лекции 7, 8
# test_lecture_5 <- read_xlsx("data/Конкуренция. Трофические связи (Общая экология)(1-79).xlsx")
# test_lecture_5 <- test_lecture_5 %>% select(1,2)
# names(test_lecture_5) <- c("St_Mail", "Score_lecture_5")
# 
# credits <- merge(credits, test_lecture_5, all.x = TRUE)
...
# #Тест по Лекции 9, 10
# 
# test_lecture_6 <- read_xlsx("data/Тест по материалам лекций № 9 (симбиозы) и № 10 (структура сообщества) (Общая экология)(1-80).xlsx")
# test_lecture_6 <- test_lecture_6 %>% select(1,2)
# names(test_lecture_6) <- c("St_Mail", "Score_lecture_6")
# 
# test_lecture_6$Addition_4 <- 1
# test_lecture_6$Addition_5 <- NA
# test_lecture_6$Addition_5[test_lecture_6$St_Mail == "st075764@student.spbu.ru"] <- 2
...
credits_print$Exam <- round(credits_print$Exam, 1)*(10/8) #умножаем на это число, так как от каждой из 8 лекций приходит 10/8 финальной оценки
...
<!-- hw1_score$Exam <- round(hw1_score$Total * 1, 1) -->
...
cont1_print <- merge(class, cont1 %>% select(-Family) ) %>% select(-St_Mail) %>% mutate(Exam = Correct / 45 *10)
...
**Коллеги!** В принципе, контрольную вы написали хорошо. НО! Я при постановке оценки решил, на первый раз, не учитывать штрафы. Посмотрите внимательно на свой результат. И пересчитайте свою оценку с учетом штрафа. А главное, посмотрите на чудесный график. Корреляция между временем, затарченным на тест, и количеством ошибок вполне статистически значимая (R = `r round(correl$estimate, 2)`, p = `r round(correl$p.value, 4)`). Не торопитесь! 

```{r warning=F, message=F}
qplot(y =cont1$Incorrect, x= cont1$Time) + geom_smooth(method = "lm") + labs(x = "Время выполнения теста (минуты)", y = "Количество ошибок")
```
