...
test_lecture_2 <- read_xlsx("data/Аутэкология (Общая экология для биоинженеров 2022)(1-26).xlsx")
...
test_lecture_3 <- read_xlsx("data/Структура популяций  (Общая экология для биоинженеров 2022)(1-26).xlsx")
...
test_lecture_4 <- read_xlsx("data/Структура и динамика популяций  (Общая экология для биоинженеров 2022)(1-28).xlsx")
...
test_lecture_5 <- read_xlsx("data/Взаимоотношения организмов (Общая экология для биоинженеров 2022)(1-26).xlsx")
...
credits_print <- credits_print %>% select(-Sum, -Credit_Sum)
...
cont1_print <- merge(class, cont1 %>% select(-Name ) ) %>% select(-St_Mail) %>% mutate(Exam = round(Score / 21 * 3, 1) ) %>% select(-Family, -Name, -Otch, -Group )
...
sem <- sem %>% select(Login, Seminar_1)

sem_print <- merge(class, sem) %>% select(Login, Group, Seminar_1) %>% mutate(Exam = Seminar_1 * 8) 
...
cont2_print <- merge(class, cont2 %>% select(-Name ) ) %>% select(-St_Mail) %>% mutate(Exam = round(Score / 13*3, 1))
