...
# 
# 
# test_lecture_7 <- read_xlsx("data/Взаимоотношения организмов (Общая экология для биоинженеров 2023)(1-10).xlsx")
# 
# 
...
hw1_score$Exam <- round((hw1_score$Score + hw1_score$Add_Score) / hw1_score$Total * 1.5, 1)
...
<!-- sem_print <- merge(class, sem, all.x = TRUE ) %>% select(Login, Seminar_1, Seminar_2) %>% rowwise() %>% mutate(Exam = sum(Seminar_1 * 8,  Seminar_2 * 8, na.rm = T) ) %>% filter(Exam != 0) -->
