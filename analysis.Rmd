...
# Тест по Лекции 1
test_lecture_1 <- read_excel("data/Основы ОТС и предмет Экологии (Общая экология 2023)(1-110).xlsx")
...
test_lecture_2 <- read_xlsx("data/Аутэкология (Общая экология 2023)(1-108).xlsx")
...
test_lecture_3 <- read_xlsx("data/Структура и динамика популяционных группировок (Общая экология 2023)(1-103).xlsx")
...
credits_sum <- 9 + 10 + 22 + 10 + 9 + 6 + 7 + 8
# + 10 + 8 + 10 + 10 + 5 + 7
...
<!-- cont1 <- read_excel("data/Контрольная работа № 1 (Общая экология 2023)(1-111) (1).xlsx") -->

<!-- cont1 <- cont1 %>% select(1:3) -->

<!-- names(cont1) <- c("St_Mail","Name", "Score") -->

<!-- cont1_print <- merge(class, cont1 %>% select(-Name ), all.x = T ) %>% select(-St_Mail) %>% mutate(Exam = round(Score / 22 * 1.5, 1) ) -->

<!-- # cont1_print <- merge(class, cont1 %>% select(-Name ), all.x = T ) %>% select(-St_Mail) %>% mutate(Exam = round(Score / 19 * 3, 1) ) -->
