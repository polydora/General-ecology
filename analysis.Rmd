...
# test_lecture_7 <- test_lecture_7 %>% select(1,2)
...
cont1 <- cont1 %>% select(-Name)

names(cont1) <- c("St_Mail", "Score")

cont1 <- cont1 %>% mutate(Total = 24, Exam = Score/Total)

cont1_print <- cont1

# 
# cont1_print <- merge(class, cont1 %>% select(-Family) ) %>% select(-St_Mail) %>% mutate(Exam = Correct / 45 *5)
# 
...
По поводу вопроса № 25. В задании теста была очевидная ошибка.  При -35 радусах, естественно, развитие растений проходить не может. НО! правильный ответ, все-таки, был - это не ставить вообще никакого решения. Поэтому, никому я этот вопрос не зачел, кроме тех, кто вообще на него не поставил ответа. Вот  эти люди:
...
Им я добавил +1. НО! чтобы за эту подставу обидно не было и остальным, я общую сумму баллов уменьшил до 24. 
...
<!-- cont2 <- read_excel("data/Control_work_2_biotech.xlsx") -->

<!-- cont2_print <- merge(class, cont2 %>% select(-Name) ) %>% select(-St_Mail) %>% mutate(Exam = round((Correct - Incorrect) / 54 *5, 3)) -->
...
<!-- d2$Exam[is.na(d2$Exam)] <- 0 -->


<!-- d3 <- sem_1_print %>% select(Login, Exam)  -->

<!-- d4 <- sem_2_print %>% select(Login, Exam)  -->

<!-- d5 <- cont2_print %>% select(Login, Exam) -->


<!-- Results <- merge(d1, d2, by = "Login") %>% merge(., d3, by = "Login") %>% merge(., d4, by = "Login") %>%  merge(., d5, by = "Login")  -->


<!-- names(Results) <- c("Login", "Lecture_test (max 10)", "Control_1(max 5)", "Seminar_1 (max 5)", "Seminar_2 (max 5)", "Control_2 (max 5)") -->


<!-- Results_print <- Results -->

<!-- Results_print$Exam <- round(apply(Results[,-1], MARGIN = 1, FUN = sum ), 1)  -->
