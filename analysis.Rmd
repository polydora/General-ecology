...
cont2 <- read_excel("data/Контрольная работа № 2 (Общая экология 2024-2025).xlsx")

cont2 <- cont2 %>% select(4:6)

names(cont2) <- c("St_Mail",  "Name", "Score")

cont2_print <- merge(class, cont2 %>% select(-Name ), all.x = TRUE ) %>% select(-St_Mail) %>% mutate(Exam = round(Score / 19 * 1.5, 1))
