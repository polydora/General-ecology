...
reasult_contr2 <- read.csv("data/Control_work_1.csv", encoding = "Utf-8")
 

tmp <- merge(reasult_hw1, reasult_hw2, by = c("user_id", "last_name", "first_name"))
all_hw <- merge(tmp,reasult_hw3, by = c("user_id", "last_name", "first_name"))

Total_sum_hw <- sum(all_hw[1,-c(1:3)])

all_hw$sum <- apply(all_hw[,-c(1:3)], MARGIN = 1, FUN = sum)


all_contr <- merge(reasult_contr1, reasult_contr1, by = c("user_id", "last_name", "first_name"))

Total_sum_contr <- sum(all_contr[1,-c(1:3)])
...
kable(result_total, col.names = c("Ф. И.", "Группа", "Контрольные (макс. 46)", "Самостоятельные (макс.77)"), align = "lccc", row.names = FALSE)
